#!/usr/bin/env python3
"""
Gemma 4 JD Parser — API Server
Flask API for parsing job descriptions using Google's Gemma 4 (via Google AI Studio).
...
from gemma_jd_parser import parse_jd, extract_text_from_file, is_google_configured, GOOGLE_MODEL
...
def parse_bulk():
    ...
    # Parse concurrently with a small worker pool to respect API rate limits
    results = []
...
if __name__ == "__main__":
    ...
    print(f"Starting Gemma 4 JD Parser on port {port}")
    print(f"Model: {GOOGLE_MODEL}")
    print(f"Google AI Studio: {'configured' if is_google_configured() else 'NOT SET'}")
